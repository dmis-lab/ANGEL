...
def replace_after_token(tensor, token):
    # 각 행마다 특정 토큰 이후의 인덱스 찾기
    indices = torch.tensor([torch.nonzero(row == token).item() if token in row else row.size(0) for row in tensor])

    # 각 행마다 특정 토큰 이후의 값을 -100으로 바꾸기
    ...
    def forward(
        ...
        **kwargs,
        ...
        if encoder_outputs is None:
            ...
            #     decoder_input_ids = replace_after_token(labels, 2)
