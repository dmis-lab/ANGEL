...
def reform_input(inputs, labels, eos_token = 2):

    enc_max_idx = torch.max(torch.where(inputs==eos_token)[1]) + 1
    dec_max_idx = torch.max(torch.where(labels==eos_token)[1]) + 1

    return enc_max_idx, dec_max_idx
...
def construct_arguments():
    ...
    # tokenizer = Tokenizer.from_file(args.config['bart_model_file'] + '/tokenizer.json')
...
def train(args,
    ...
    for _, batch_index in enumerate(tqdm(dataset_iterator, smoothing=1)):
        ...
        enc_max_id, dec_max_id = reform_input(input_ids, labels)

        input_ids = input_ids[:,:enc_max_id]
        attention_mask = attention_mask[:,:enc_max_id]
        labels = labels[:,:dec_max_id]
        decoder_input_ids = decoder_input_ids[:,:dec_max_id]
        decoder_attention_mask = decoder_attention_mask[:,:dec_max_id]
        ...
        if args.label_smoothing_factor > 0:
            ...
            loss = model.label_smoother(output, labels)
...
def run(args, model, optimizer, start_epoch):
    ...
    for index in range(start_epoch, config["training"]["num_epochs"]):
        ...
        if index % 50 == 0 and index != 0:
            save_checkpoint_model(PATH=args.saved_model_path,
                                    ckpt_id='epoch_index_{}'.format(global_step, index+1),
                                    model=model,
                                    last_global_step=global_step,
                                    last_global_data_samples=global_data_samples,
                                    args = args,
